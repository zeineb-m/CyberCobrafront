...
export default function UsersPage() {
  ...
  const handleSaveUser = async () => {
    ...
    try {
      ...
      const url = editingId
        ? `http://127.0.0.1:8000/api/auth/users/${editingId}/`
      ...
      if (res.ok) {
        ...
        await fetchUsers() // 🔹 Recharge automatique après ajout/édition
  ...
  const handleDelete = async (id) => {
    ...
    try {
      ...
      if (res.ok) await fetchUsers() // 🔹 Recharge automatique après suppression
...
const filteredUsers = users.filter(
  u =>
    !searchTerm || // si pas de recherche, tout afficher
      ...
      <div className="max-w-6xl mx-auto">
        ...
        <Modal isOpen={showModal} onClose={() => setShowModal(false)} title={editingId ? "Edit User" : "Add New User"}>
          <div className="space-y-4">
            ...
            {/* Password optionnel pour édition */}
