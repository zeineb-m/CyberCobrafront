...
export function AuthProvider({ children }) {
  ...
  // Charger token et user depuis sessionStorage/localStorage au démarrage
  useEffect(() => {
  ...
  const logout = async () => {
    try {
      ...
      if (refresh && token !== "static-token-demo") {
        await axios.post(BACKEND_URL + "/api/auth/logout/", { refresh: refresh });
      }
    } catch (err) {
